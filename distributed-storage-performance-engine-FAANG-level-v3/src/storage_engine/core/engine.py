...
class StorageEngine:
    ...
    def put(self, key, value, request_id=None):
        ...
        try:
            ...
            return {"status": "stored", "partition": self.partitioner.partition(key), "replicas": replicas}
        except Exception:
            if self.metrics: self.metrics["errors"] += 1
            raise
        finally:
            if self.metrics: self.metrics["requests"] += 1
